...
class SemanticAnalyzer(ASTVisitor):
    ...
    def visit_declaration(self, node: DeclNode):
        if node.variable in self.context.declared_variables:
            raise ValueError(
                f"Variable '{node.variable}' has already been declared at line {node.line}!!!!!!!!!!")

        self.context.declared_variables[node.variable] = node.mutable
        self.context.variable_types[node.variable] = node.data_type
        ...
        self.context.initialized_variables.add(node.variable)
        self.context.currently_initializing = None

    def visit_assign(self, node: AssignNode):
        if node.variable not in self.context.declared_variables:
            raise ValueError(
                f"Variable '{node.variable}' at line {node.line} is not declared, bro!")

        if not self.context.declared_variables[node.variable]:
            raise ValueError(
        ...
        data_type = self.context.variable_types[node.variable]
        expr_type = node.expr_node.accept(self)
    ...
    def visit_id(self, node: IDNode) -> DataType:
        ...
        if node.value not in self.context.initialized_variables:
            raise ValueError(
        ...
        return self.context.variable_types[node.value]
